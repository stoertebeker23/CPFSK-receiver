...
Das in Abschnitt~\ref{sec:rechteck} generierte Reckecksignal wird nun \ac{CPFSK} Modeliert.
...
Dafür wird in \textit{MATLAB} ein mit $N$-Nullen gefülltes Array verwendet. In Abb.~\ref{fig:comb} ist zu sehen,
...
\end{figure}
\subsection{ANSI C}
Für die C-Implementierung, siehe \ref{codeC:comb}, wird jedes Eingabe Sample um $N = 5$ verzögert.
...
\subsection{MATLAB}
In listing~\ref{codeM:decode}, der MATALB Implementierung des Decoders wird die durch den Verzögerungsdemodulator binarisierte Ausgabe
Blockweise aus dem Buffer \textit{binarized} ausgelesen. Sobald die Start/Stop Bit-Sequenz $[1 1 1 0 0]$ erkannt wird, werden die Bits aus dem Buffer
in der Reinfolge umgedreht, da hier das MSB zuerst gesendet wird. Die Bits werden in den Datentyp String umgewandelt und anschließend
